...
function pagedata(regno) {
  ...
  xhr.open("POST", "individualattendance.php", true);
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      if (response.success) {
        ...
        usernamecontainer.textContent = response.name;
...
<td><img ${hour2}><span class="tooltiptext">
  ...
  xhr.send(`regno=${regno}&submit=pagedata`);
}

pagedata(window.location.search.slice(1).split("=")[1]);